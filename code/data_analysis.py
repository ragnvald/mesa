...
class AssetAnalyzer:
    ...
    def available_geocode_categories(self) -> List[str]:
        categories: List[str] = []
        flat: Optional[gpd.GeoDataFrame]
        try:
            flat = self._load_flat_dataset()
        except FileNotFoundError:
            debug_log(self.base_dir, "No geocode categories available because tbl_flat.parquet is missing")
            flat = None
        if flat is not None and not flat.empty:
            if "name_gis_geocodegroup" in flat.columns:
                cats = flat["name_gis_geocodegroup"].dropna().astype(str).str.strip()
                categories.extend(cats.tolist())
            else:
                debug_log(self.base_dir, "tbl_flat.parquet missing name_gis_geocodegroup column; no categories available from parquet")
        if _MBTILES_INDEX:
            categories.extend(list(_MBTILES_INDEX.keys()))
        if not categories:
            return []
        ordered = sorted({c for c in categories if c}, key=lambda s: s.lower())
        debug_log(self.base_dir, f"Detected geocode categories: {ordered}")
        return ordered

    def _ensure_category(self, category: Optional[str]) -> str:
        options = self.available_geocode_categories()
        if not options:
            return DEFAULT_ANALYSIS_GEOCODE
        if not category:
            category = DEFAULT_ANALYSIS_GEOCODE
        cat_norm = category.strip()
        if cat_norm in options:
            return cat_norm
        # fallback case-insensitive match
        lower_map = {opt.lower(): opt for opt in options}
        return lower_map.get(cat_norm.lower(), options[0])
    ...
    def _flat_for_category(self, category: str) -> gpd.GeoDataFrame:
        ...
        if not category:
            ...
            if "name_gis_geocodegroup" not in dataset.columns:
                subset = dataset.iloc[0:0].copy()
            else:
                subset = dataset[dataset["name_gis_geocodegroup"].astype(str) == category].copy()
    ...
    def _write_analysis_output(self, path: Path, group_id: str, new_gdf: gpd.GeoDataFrame) -> None:
        ...
        if "analysis_group_id" in existing.columns:
            existing = existing[existing["analysis_group_id"] != group_id].reset_index(drop=True)
    ...
    def run_group_analysis(
        ...
        category = self._ensure_category(geocode or group.default_geocode or DEFAULT_ANALYSIS_GEOCODE)
...
class WebApi:
    ...
    def run_analysis(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            geocode = payload.get("geocode") or group.default_geocode
    ...
    def load_canvas(self, payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        try:
            category = (payload or {}).get("category") or self._active_geocode
...
<head>
  ...
  <style>
    ...
    .wrap { height:100vh; display:grid; grid-template-columns: 360px 1fr; grid-template-rows: 100%; grid-template-areas: "panel map"; }
    ...
    .panel h2 { font-size:14px; margin:16px 0 8px; text-transform:uppercase; letter-spacing:0.03em; color:#334155; }
...
<div class="wrap">
  <div class="panel">
    ...
    <div>
      <h2>Background</h2>
      <div class="form-group">
        <label for="geocodeSelect">Geocode category</label>
        <select id="geocodeSelect"></select>
      </div>
      <button id="refreshGeocodeBtn" title="Reload background layer">Update background</button>
    </div>
