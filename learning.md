...
## Windows taskbar icon needs an explicit AppUserModelID (2026-07-20)

- Rule: every MESA process that shows a window (the main `mesa.exe` and each separate helper exe: combined_map, segmentation_setup, segmentation_run, special_focus) calls `mesa_shared.set_windows_app_user_model_id()` — which sets `SetCurrentProcessExplicitAppUserModelID("com.mindland.mesa")` — early in its `__main__`, before any window is shown. In-process helpers (asset/atlas/geocode manage, processing_setup, processing_pipeline_run, report_generate, analysis_present) inherit the main process's id, so they need nothing. All processes share the SAME id so their taskbar buttons group under one MESA icon.
- Why: a compiled PyInstaller app came up with a blank/generic taskbar icon even though the exe had an embedded icon AND `setWindowIcon(mesa.ico)` succeeded (verified: `system_resources/mesa.ico` ships next to the exe and `resolve_path` finds it because PROJECT_BASE = the exe dir when frozen). Without an explicit AppUserModelID, Windows derives the taskbar identity from the process/loader and does not associate it with the window icon, so the button is iconless. `setWindowIcon` alone drives the title-bar icon but is not sufficient for the taskbar button in a frozen app. Confirmed the fix with a ctypes round-trip: after the call, `GetCurrentProcessExplicitAppUserModelID` returns `com.mindland.mesa` (HRESULT 0).
- How to apply: any NEW separate-process helper that opens a window must call `set_windows_app_user_model_id()` at the top of its `__main__`. It is a stdlib-only, Windows-guarded no-op elsewhere, living in mesa_shared so there is one source of truth.
- Non-regression guarantee: the call is fully guarded (try/except, `sys.platform != "win32"` short-circuit) so it cannot break startup on any platform; it only sets a taskbar-grouping identity and changes no behaviour beyond the icon association.
