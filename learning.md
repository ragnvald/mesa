...
- Rule: macOS and Windows both target CPython 3.14. Mac dev/runtime venv = `python3.14 -m venv .venv` + `requirements_py314.txt`; Windows = `requirements_py314_win.txt` (same versions, pyobjc dropped, EdgeChromium webview). `requirements_macos_dev.txt` (the pre-migration 3.11 set) is DELETED — do not reintroduce a 3.11-specific requirements file. [SUPERSEDED 2026-07-23: the two base files are merged into a single cross-platform `requirements_py314.txt`; the six pyobjc-* pins carry `; sys_platform == "darwin"` so pip installs them on macOS and skips them on Windows. `requirements_py314_win.txt` is deleted; both platforms and the compile files use the one base.]
...
## Boundary-bleed filter must fail per row, not per batch (2026-07-21)

- Rule: `_drop_boundary_only_joins` (`processing_internal.py`) isolates geometry failures per row — the vectorised `shapely.intersection` is retried row-by-row (with a `make_valid` retry per pair) when it raises, rows that still cannot be evaluated are KEPT but counted, and every fallback appends a message to the worker's `logs` list. Never let this function swallow an exception silently.
- Why: the old `except Exception: return res` returned the WHOLE batch unfiltered, so one bad geometry republished exactly the boundary bleed the function exists to remove — and said nothing. The trigger was MESA's own self-intersecting `basic_mosaic` faces (see "Self-intersecting basic_mosaic faces come from the reprojection"): shapely is NOT tolerant here, `shapely.intersection` over the 39 sjoin rows touching the 9 invalid faces raises `GEOSException: TopologyException: side location conflict`. Measured cost on the 5.5.0 Uganda fixture, comparing the shipped `tbl_flat` against a full rerun with both fixes: **342 of 26,975 `basic_mosaic` cells (1.27%) had `sensitivity_max` too high, 340 changed A–E class, all downward** (56 of them out of A or B; mean drop 4.9, max 16), and 0 cells were too low — the error only ever inflates. The other seven geocode layers were bit-identical (0 cells changed), since only `basic_mosaic` has asset boundaries as cell boundaries. The extra rows were pure boundary contacts: 266 with zero overlap, 885 with a median overlap of 0.000008 m².
- How to apply: any vectorised geometry op over a batch inside a worker needs to degrade per element, not per batch — a batch-wide `except` turns a local defect into a silent, geographically clustered data error (here 1,151 bad rows sat in 2 of 9 chunks, which reads like a real spatial pattern rather than a bug). If a fallback cannot evaluate a row, keeping it is the safe choice, but the count MUST reach the operator. When touching this function, re-run the fixture comparison rather than trusting row counts: `tbl_stacked` row totals barely move while `sensitivity_max` shifts a full class.
- Non-regression guarantee: the healthy path is unchanged — when nothing raises, the vectorised branch runs exactly as before and emits no log lines (verified: the post-fix pipeline run produced zero boundary-bleed messages). Only batches that would previously have skipped filtering entirely now behave differently, and they can only remove rows the filter was always meant to remove.

## Geometry validity is repaired at every entry point, never dropped silently (2026-07-21)

- Rule: invalid geometry is repaired with `make_valid` (`buffer(0)` fallback) wherever it enters or leaves a stage, and the count is logged. Asset import (`asset_manage`, `import_validate_geometries`, now **true** by default and present in `config.ini`), geocode import (`geocode_manage._repair_imported_geometry`, both the vector and parquet readers), mosaic faces (`_repair_faces`) and line segments (`processing_pipeline_run._repair_invalid_segments`). Never `gdf[gdf.is_valid]` — dropping is silent data loss.
- Why: MESA's own invalid `basic_mosaic` faces inflated `sensitivity_max` in 1.27% of cells (see "Boundary-bleed filter must fail per row"), and the same class of defect can arrive from an imported layer, where nothing was checking. Three gaps were closed at once: (1) `import_validate_geometries` was hardcoded `false` in two places and absent from `config.ini`, so the visible "Validate geometries" checkbox was off unless a user ticked it — while `build_user_guide.py` and `code/README.md` both told users every geometry passes through `make_valid`; the docs were aspirational, not descriptive. (2) Geocode import repaired nothing at all, and an imported geocode layer becomes the cells assets are intersected against. (3) `processing_pipeline_run` did `segs = segs[segs.is_valid]`, deleting invalid segments — each one a hole in that line's sensitivity profile that nothing downstream reports.
- How to apply: after repairing, check the geometry TYPE, not just validity. `make_valid` on a collapsed polygon returns a Point — valid, non-empty, zero area — which would ride on into the segment intersect as a bogus row; `_repair_invalid_segments` keeps only Polygon/MultiPolygon and reports the rest as dropped. Prefer `make_valid` over `buffer(0)`: on a bowtie, `buffer(0)` silently keeps one lobe (area 1.25 in a synthetic test) while `make_valid` keeps both (1.5). Extract repair blocks to module level rather than nesting them inside a stage function — the three helpers here are unit-testable precisely because they are not closures.
- Non-regression guarantee: the healthy path is untouched everywhere — no invalid geometry means no repair, no log line, and identical output (verified: the lines stage produced the same 78 segments before and after the refactor). Turning asset validation on by default costs ~1 s per 800k vertices (measured: 995 ms over the Uganda fixture's 16 assets / 804,888 vertices) and changes area by exactly 0.0 m². Users who deliberately unticked "Validate geometries" keep their choice — `config.ini` wins over the new default.

## Dead per-group ranking in index_owa, and a test suite nobody ran (2026-07-21)

- Rule: `_compute_index_owa_from_counts` ranks over the WHOLE project — every geocode group pooled. It no longer takes a `labels` argument, because the one it had never worked. The published wiki still describes per-group ranking, so code and docs disagree until the method question below is settled — do not "fix" either side alone. Run `pytest tests/ -q` (deps in `requirements_dev.txt`) before a release; the suite is not wired into CI.
- Why: the call site built `group_map` indexed by `code` (`basic_mosaic_027213`) and the function did `labels.reindex(df.index)` against `tbl_flat`'s integer RangeIndex. Measured: **0 of 48,615 labels matched**, everything fell to `.fillna("__all__")`, and the `groupby` saw one group. The parameter had been inert since it was written, so the code read as if it ranked per group while behaving as if it did not — and the published wiki says per-group, so the documentation has been describing an intention rather than the code for as long as the parameter has been dead. Two things kept it invisible: `except Exception: tbl_flat["index_owa"] = 0` wrote a silently zeroed analytical column that no reader can distinguish from "no overlaps anywhere", and the `drop_duplicates(subset=["code"])` guard defended against duplicate labels in `reindex` — the one condition that WOULD have raised and exposed the mismatch. The defensive code addressed the failure mode that could not happen instead of the one that did. Separately, `tests/test_processing_internal.py` had been failing since 5.2 (it referenced `INDEX_WEIGHT_DEFAULTS`, removed then, and passed 4 args to a 3-arg worker), and `pytest` was not installed in `.venv` nor listed in any requirements file: the suite had not run in months, and there is no `.github/workflows` at all.
- How to apply: when a function takes a grouping/label argument, assert that it matched something — `labels.reindex(index).notna().any()` — rather than defaulting silently to one bucket; a `fillna` on a join key hides a total mismatch just as well as a partial one. Never write a computed analytical column inside a bare `except` that substitutes zeros: fail, or log loudly enough that the operator knows the column is not data. `requirements_dev.txt` is deliberately separate from the runtime and compile requirement sets so pytest never reaches a PyInstaller build.
- Non-regression guarantee: the refactor is value-preserving by construction — the removed loop ran exactly once over all rows. Verified against the pre-change implementation reconstructed from git on five randomised populations (1 to 20,000 rows, 0–100% all-zero rows): outputs identical in every case, including the 0-for-all-zero and single-row edges. Whether the ranking population SHOULD be per group is a method question, deliberately left open — changing it would move every published `index_owa` value (measured elsewhere: 19.9% agreement overall, 0.13% on `basic_mosaic`) and is worth doing once, together with the pending question of rank formulation, not twice.

## Verify a documentation claim against the committed file, not the working copy (2026-07-21)

- Rule: when a doc's wording is the PREMISE of a conclusion — "the code matches the docs", "this was already fixed" — read it with `git show HEAD:<file>` and check `git log -1 -- <file>` and `git status` first. A working copy is not evidence of what is published.
- Why: reviewing a field report that said the wiki documented per-group `index_owa` ranking while the code pooled, I opened `mesa.wiki/Indexes.md`, found it describing pooled ranking in three places including a callout on the exact consequences, and concluded the report's finding was outdated — telling the reporters to check they were reading the newest wiki. The text I read was an UNCOMMITTED edit the reporters had made in that working copy hours earlier; `git show HEAD~1:Indexes.md` still says "within each geocode group" on lines 23 and 68, and the last commit touching the file (2026-06-09) removed two index sections without touching the pooling statement. The conclusion inverted: code and published docs DO disagree. The wrong claim then propagated into a docstring, a learning.md entry and a commit message before it was caught by the people I had contradicted.
- How to apply: `git log -1 --format="%h %ad %s" --date=short -- <file>` costs one command and would have shown the dates did not support the story. Treat a mismatch between "what the file says" and "what the history says was changed" as the signal to stop. This applies with double force to sibling repos (`mesa.wiki`) and shared disks, where another person or agent may be mid-edit in a checkout that looks like yours: `git status` there is part of reading the file. When a correction lands, fix every place the wrong claim reached — code comments and commit messages included; unpushed commits can be soft-reset and rewritten, which is cheaper than a permanent falsehood in the history.

## Generated per-geocode QGIS results project (2026-07-22)

- Rule: `code/qgis_results_project.build_results_project` writes `qgis/mesa_results.qgz` by cloning the shipped `qgis/mesa.qgz` template and adding styled vector layers per geocode group found in `tbl_flat` (Sensitivity, Importance, OWA). It runs best-effort at the end of a Process run, from the `finally` of `_auto_run_tiles_stage` in `processing_internal.py`, and must never raise into the run. The seed template is never modified. The generated `qgis/mesa_results.qgz` is tracked and ships alongside `mesa.qgz` (a default that self-refreshes on each Process run) — so a Process run leaves it modified in git, by design.
- Why: the seed only carries the always-present `basic_mosaic` results, but a project's geocode layers (H3/QDGC levels, uploaded admin layers) vary run to run, so their result layers cannot be frozen into a static template — they must be generated against the actual `tbl_flat`. All geocode groups live in one `tbl_flat.parquet` keyed on `name_gis_geocodegroup`, each carrying `sensitivity_code_max`, `importance_code_max` and `index_owa`, so a per-group styled layer is the same renderer with a different subset filter. The `.qgs` is edited with ElementTree (add `<maplayer>` to `<projectlayers>`, a `<layer-tree-layer>` to a new group under "Results (step 4)", and an `<item>` to `<custom-order>`); QGIS links tree node to layer by id, not by source, and rebuilds legend/snapping from the tree, so those three sections are the minimum. Re-add the `<!DOCTYPE qgis ...>` line ElementTree drops on write.
- How to apply: clone renderers rather than authoring them — the seed's proven Sensitivity rule renderer (5 rules on A–E) is the single style reference; Importance reuses it with the filter column swapped to `importance_code_max`, and OWA rebuilds its `<rules>`/`<symbols>` (5 bins on `index_owa`) from the seed's fill symbol. Deep-copy the reference element per clone (`copy.deepcopy`) so mutating one never touches the seed or a sibling. NOTE a latent seed bug this surfaced and fixed: the template's own "Importance screen/map production" layers filtered on `sensitivity_code_max`, so the shipped basic_mosaic Importance vector was coloured by sensitivity. Fixed in `qgis/mesa.qgz` by swapping the column inside those two layers' filter attributes only (byte-exact edit, CRLF preserved — read/write in binary or `newline=''`, else every line shows as changed). "Importance segments" (tbl_segment_flat) still keys on sensitivity because that table has no `importance_code_max`, only numeric `importance_max` — a proper fix there needs a numeric-range renderer, deferred.
- Non-regression guarantee: purely additive and side-effect-free — the seed `qgis/mesa.qgz` is read-only, the generator only ever writes a separate `qgis/mesa_results.qgz`, and a failure (missing seed, missing `tbl_flat`, no groups) logs and returns None without touching run status. Verified on the Uganda fixture: 24 layers across 8 groups, XML re-parses, all ids unique, every subset returns rows, Sensitivity/Importance clones are byte-identical to the seed renderer (with the documented attribute swap for Importance), and OWA carries the five `index_owa` filters. Importance renders uniformly class E there because the demo land-cover data is uniformly E on importance — a data property, not a renderer fault. **SUPERSEDED 2026-07-30: that last sentence was wrong.** The uniform class E was not a data property; `importance_code_max` was the letter E in every row of every project, because the single factors were banded on the sensitivity ranges. See "Single-factor columns banded on the product's scale" below. The renderer was fine; its input was not.

## Restore must place packaged qgis/ files; it silently dropped them (2026-07-22)

- Rule: `restore_backup_archive` (mesa.py) extracts members under `config.ini`, `input/`, `output/`, `secrets/`, the demo readme, **and `qgis/`**. A package (e.g. a mesa-server demo/export) that ships `qgis/mesa_results.qgz` needs it landed at `<workspace>/qgis/mesa_results.qgz`. Convention for producers: put the file at exactly `qgis/mesa_results.qgz` in the archive — restore matches on that prefix; a file at the zip root or under another path is ignored.
- Why: the extraction whitelist is an explicit allowlist of path prefixes, so anything outside it is dropped without warning. `qgis/` was absent, so a bundled results project vanished on restore. Unlike `input/` and `output/`, the restore does NOT rmtree `qgis/` first — that folder holds the install's shipped `mesa.qgz` template, which is not project data. So `qgis/` members overwrite in place: a packaged `mesa_results.qgz` lands, a packaged `mesa.qgz` would update the template, and anything the package omits is left untouched. Verified on a synthetic package: `qgis/mesa_results.qgz` placed, a pre-existing `qgis/mesa.qgz` preserved, `output/` replaced, `config.ini` restored.
- How to apply: when adding a new shippable folder to MESA, update BOTH sides — `create_backup_archive` (does it go into packages?) and the `restore_backup_archive` whitelist (does it come back out?). They are independent lists and it is easy to add one and forget the other. Desktop backups still do not bundle `qgis/` (the results project regenerates on the next Process run), so this fix is restore-only and asymmetric by design; revisit if desktop packages should carry the results project too.

## Opening old backups in a newer MESA: schema is stable, values are not (2026-07-23)

- Rule: `restore_backup_archive(base, zip, mode=..., stamp_version=...)` in mesa.py handles cross-version restore. `_do_restore` reads the archive's `mesa_version` (`_archive_mesa_version`, reusing the line-based `_read_config_mesa_version`), compares with `_version_is_older`, and if older offers **Re-import** (restore input/ + config, skip output/, stamp version → operator runs Process) or **Upgrade in place** (keep stored output/, stamp version). Input-only backups (`_archive_has_geoparquet_output` False) get re-import automatically — there is nothing to keep. Same-version restores are unchanged.
- Why: the task assumed output schema drift makes old backups "half-unusable". A real column diff of 5.0.3 / 5.3 / 5.5.0 fixtures against what 5.6.0 reads disproved that: for every table the old backup is a column *superset* of what the reader needs — no column 5.6.0 requires is missing (5.0.3 merely carries the removed `index_importance`/`index_sensitivity`, and old `tbl_stacked` has extra columns). Extra columns don't break reading. So schema migration is a near no-op for 5.3+; the genuine cross-version difference is **values** (this cycle's sensitivity_max/index_owa fixes), and only re-import (regenerating output from raw inputs) yields current-version-correct numbers. That is exactly the path mesa-server uses for its input-only exports. Hence: re-import is the substantive mechanism; "upgrade" is a light stamp + honest "these numbers predate fixes" message, not a column rewriter.
- How to apply: when adding a cross-version data path, DIFF real old fixtures before writing a migrator — the migration you assume you need may not exist. Old-version fixtures live in `D:/data/mesa_demodata` (5.0.3 in the repo's `test_mesa_backup_*.zip`, 5.3 `mesa_backup_2026-07-13_*.zip`, 5.5.0 `fixtures/mesa_5.5.0_uganda_*` + `exported/*`). Stamp the version by targeted line-rewrite (`_stamp_config_version`), never a configparser round-trip — config.ini uses `#` as hex-colour DATA and a plain parser corrupts it. Re-import must exclude output/ from the extract list AND rely on the existing input+output rmtree, so output/ ends empty.
- Non-regression guarantee: same-version restores take the unchanged `mode="as_is"` path (no dialog, no stamping). Verified on fixtures: reimport of a 5.3 backup left 0 output parquet files with 48 inputs restored and `mesa_version` stamped to 5.6.0; upgrade of a 5.5.0 backup kept 33 output parquet files and stamped 5.6.0. Residual: `tbl_seg_mv`/`tbl_lines`/`tbl_segments`/`tbl_project_info` couldn't be diffed against a true 5.6.0 tree (the repo output lacks them) — but no code touched their schema this cycle; confirm against a real 5.6.0 run when one exists.

## Progress map stuck as "already open" (2026-07-25)

- Rule: The processing "Progress map" subprocess (`_map_process_entry` in `code/processing_internal.py`) ends with `os._exit(0)` after `webview.start()` returns, so closing the window terminates the process at once.
- Why: `open_minimap_window()` gates re-opening solely on `_MAP_PROC.is_alive()` — no separate flag, no on-closed reset. The gate is one-sided and has no timeout, so if the map process lingers after the window is visually closed, the guard stays stuck on "Minimap is already open" until the whole processing window is closed. It could linger because the OSM tile proxy feeding the map is a `ThreadingHTTPServer` without `daemon_threads=True` (`code/mesa_osm_tiles.py`): the `serve_forever` thread is daemon, but each in-flight tile-request handler thread is NOT, and interpreter shutdown joins them — a slow/hung OSM fetch (likely right after a run, when the map was actively loading tiles) blocks exit. Reported symptom: cancel a run, close the map, start again, ask for the map → "already open" with no window present. `os._exit` skips the thread-join entirely; the `finally` above it already releases the proxy.
- How to apply: when a window's "is it open?" state is a one-sided liveness check on a child process/thread with no reset path, make the child exit deterministically (hard-exit on close) rather than trusting graceful teardown — especially when non-daemon worker threads (HTTP handlers, network fetches) can outlive the visible window. The deeper defense (not taken here to keep the fix minimal) is `server.daemon_threads = True` on the proxy.
- Non-regression guarantee: only the map subprocess's exit path changed; it always ran to process end after the window closed anyway. The `finally` that stops the OSM proxy still runs before `os._exit`. Normal open/close is unaffected; the fix only removes the lingering-after-close case.

## Workspace relocation broke helper lookup and the Maps window (2026-07-26)

- Rule: when the workspace and the install are separate folders (frozen builds since 5.6: workspace `<Documents>/MESA`, helpers in `<install>/tools/`), every path that reaches for a *shipped* file must be resolved against the install dir, and every child process must be told the *workspace* explicitly. `_find_helper_runner` (`code/processing_pipeline_run.py`) now also searches `<sys.executable>/tools|system/<stem>.exe`, and `open_combined_map()` (mesa.py) passes `--original_working_directory` like every other launcher.
- Why: a field report on an imported mesa-server dataset — "Classification is greyed out, and segmentation results never appear on the map". Neither was about the data. `_find_helper_runner` searched `base_dir/{tools,code,system}` plus `<sys.executable>.parent/<stem>.exe`; `tools/` was only ever tried relative to `base_dir`, which used to *be* the install dir. With `base_dir` now in Documents, all five candidates missed, so `detect_classification_processing` reported "Missing: segmentation_run helper" and disabled the stage checkbox — and `detect_tiles_processing` failed identically, silently. That second failure did real damage: with Data run and Tiles unavailable, the "stale tiles" cleanup (`plan.run_data and not plan.run_tiles`) deleted 15 imported `.mbtiles` the operator had no way to rebuild. Separately, `open_combined_map()` was the one launcher not passing the working dir, so `_infer_base_dir_from_cmd` returned None, `_launch_gui_process` fell back to `PROJECT_BASE`, and the Maps window opened the demo data shipped beside the exe (H3_R7/QDGC_L6) instead of the operator's project (H3_R8/QDGC_L8) — a wrong map, not an empty one, which is why it read as "results missing".
- How to apply: after moving a writable root, grep for every consumer of the *old* combined meaning — here `base_dir` silently meant both "where the data is" and "where the program is". Two smells worth their own check: a candidate list where the install-relative layout (`tools/`) is only expressed relative to a variable that no longer points at the install, and a launcher call that differs from its siblings by a missing argument. Also: an availability probe that disables a stage must not leave a destructive cleanup enabled on the same condition — the tiles wipe assumes "unchecked" means the operator chose to skip, never "the helper could not be found".
- Non-regression guarantee: both changes are additive. The new `_find_helper_runner` candidates are appended after the existing ones, so any layout that resolved before resolves to the same path; they are `None` when not frozen. Passing `--original_working_directory` to combined_map makes it match the other eleven launchers, and in the old single-folder layout the argument equals the `PROJECT_BASE` fallback it replaces. Not fixed here, deliberately: the mbtiles wipe still fires when the tiles helper is missing, and the mac frozen layout (`system/<name>.app/...`) is still not covered by the exe-relative candidates.

## Parameter workbooks must join on name_original, not id (2026-07-28)

- Rule: an asset-group `id` is meaningful only inside one project's `tbl_asset_group`. Join parameter workbooks (`vulnerability` sheet) on `name_original`, matched exactly — whitespace stripped, case PRESERVED — and keep `id` only as a fallback for a legacy row that carries no name. Report unmatched rows in both directions; refuse values outside the valid scale rather than clamping them.
- Why: `_apply_vulnerability_from_df` merged on `id` when both sides had one. The moment a workbook is sorted, reused, or comes from another extract, its ids point at different layers, and the merge applied it in silence. Measured on Albertine Graben: 33 of 35 ids disagreed between a package's import order (`Land_Condition_Moderate=1`) and an alphabetical workbook (`Archaelogical_sites=1`); only two coincided, by chance. The same mechanism set Coral/Algae to 1/1 instead of 5/5 in the Mombasa project — the operator ran the Excel import, one row failed to match, and `sanitize_vulnerability` clipped the untouched 0 up to 1, so the most sensitive habitat in the dataset was published as the least. Case must not be folded: two layers differing only in case are two layers.
- How to apply: whenever a key crosses a project boundary — a workbook, a package, an export — ask what that key is stable *relative to*. Import order is not an identity. The paired export change is mandatory, not cosmetic: desktop seeds `importance = susceptibility = 0` at asset import, and mesa-server's reader refuses out-of-scale values, so exporting the seeded 0 makes the workbook unreadable there. Export blank for "not assessed". Mirrors mesa-server `app/param_io.py` (`_strip_name`, `join_valuations`, `read_vulnerability_xlsx`).
- Non-regression guarantee: verified against mesa-server's own acceptance test on the real 30-asset project — shuffling the `id` column now yields an identical import, which it did not before. Dropping the `id` column entirely is also identical; dropping `name_original` falls back to id and still values 30/30.

## Running MESA from source makes the repo the workspace (2026-07-28)

- Rule: when not frozen, `_resolve_working_dir()` returns `PROJECT_BASE` — the repo itself. Restore and "Clear generated data" therefore act on `d:\code\mesa`, not on `<Documents>\MESA`. Before running either from a source checkout, know which workspace you are pointed at.
- Why: a restore of `Elvegrisen_260726_1532.zip` from a source run replaced the repo's `input/` and `output/` at 17:51. Three tracked `readme.txt` placeholders vanished and the repo's sample assets were swapped for the package's. They were not in the recycle bin and never would be: `restore_backup_archive` calls `shutil.rmtree(folder, ignore_errors=True)` on `input` and `output`, which unlinks at filesystem level. Only the tracked files were recoverable (`git restore input/`); anything untracked was gone. The repo's `tbl_flat` and `tbl_stacked` are still that package's, computed by MESA 5.5.0 — which is why a fresh flatten differs from the stored file on ~2% of sensitivity codes, unrelated to any current change.
- How to apply: check `log.txt` for `base_dir=` before trusting where a source run wrote. If the repo's `output/` is a restored project rather than a generated one, say so in any measurement taken from it — otherwise a version difference reads as a regression.

## Watchdogs must not sum a metric over a changing process set (2026-07-28)

- Rule: when watching a worker pool, compare CPU time per PID across two samples and count only PIDs present in both. Never sum accumulated counters over the whole set: a worker that exits between samples takes its total with it and the delta goes negative.
- Why: three false alarms in one run, all self-inflicted, during a 25-hour mosaic. (1) A watchdog on the parent PID reported "CPU fell to 4% — it is swapping" at the exact moment the parent handed work to children and became a coordinator; the run was healthy. (2) A summed-CPU watchdog reported `-973s in 120s` when a worker finished mid-window. (3) A log-staleness watchdog with a 20-minute threshold fired on a merge that legitimately took 2h29m. Each pulled the operator into alarm over a run that was fine, which is its own cost.
- How to apply: pick the signal the work itself emits. The mosaic logs a line per completed merge, so log-file mtime with a threshold above the longest known merge was the only detector that never lied. Metrics derived from the process table are a fallback, and when used must survive processes appearing and disappearing. Also: a long silence is not evidence of a hang when the phase has no progress callbacks — verify with CPU before concluding, and verify per-PID.

## The mosaic pre-flight gate underestimates by an order of magnitude (2026-07-28)

- Rule: `mosaic_preflight_gb_per_million_assets` (default 7.0) scales the memory estimate on asset COUNT. Peak memory is driven by VERTEX count. On vertex-dense data the gate passes a job it should refuse. Do not read a green pre-flight as headroom.
- Why: Albertine Graben, 30 shapefile layers, 980,418 assets. Estimated peak `(980418/1e6) x 7.0 = 6.9 GB` against a budget of 63 GB — passed with a factor-of-nine margin. Measured peak: **85.65 GB**, with free RAM down to 2.35 GB. The tell was in the data all along: 831 MB of geometry over 980,418 polygons is ~964 bytes per polygon, and a per-column measurement shows geometry is 92% of that table against 8.3 bytes for the integer id. On the 32 GB server this job was meant for, the budget would have been ~22 GB and the estimate still 6.9 GB — the gate would have waved it through and let it swap-die, which is the opposite of its purpose.
- How to apply: when an estimator's independent variable is a row count, check whether the cost is actually per-row or per-byte. For geometry work it is almost never per-row. A cheap improvement without changing the config contract is to scale on the parquet size of `tbl_asset_object` rather than its row count. Until then, treat the gate as a floor, not a ceiling, and check geometry bytes-per-row before starting a large mosaic.

## Single-factor columns banded on the product's scale (2026-07-30)

- Rule: `[A]..[E]` in config band SENSITIVITY, which is importance x susceptibility and runs 1..25. Importance and susceptibility are single factors on `valid_input` (1..5) and need their own bands — `derive_value_class_ranges` (`code/processing_internal.py:1148`). Applying the sensitivity bands to a factor puts every value inside E's `1-5` range.
- Why: `importance_code_*` and `susceptibility_code_*` were the letter `E` in every row of every project — verified on real output, 2,722 of 2,722 rows in one and all 8,480 in another. Eight of `tbl_flat`'s 27 columns were constant and meaningless. They are not decorative: `qgis_results_project` keys an A–E rule renderer on `importance_code_max` (`_IMP_ATTR`), the shipped `qgis/mesa.qgz` references it 39 times and `susceptibility_code_max` 24 times, and Maps shows "Importance code" in the cell popup. All of it had been rendering one class for everything. This supersedes the closing sentence of "Generated per-geocode QGIS results project", which attributed the uniform class E to a data property.
- How to apply: when a derived quantity and its inputs share a classification vocabulary, check that they share a *scale* before reusing the bands. The banding cuts the project's own `valid_input` over the same letters worst-first, so 5 → A and 1 → E, and returns them in the config's A-first order — which decides the winner if bands ever overlap, since `map_num_to_code` returns the first match. Two rough edges are known and deliberate: an odd scale length puts the remainder on the last letter (1..7 gives A = 5-7 while B–E get one value each), and a scale shorter than the letter set omits the top letters entirely (1..3 yields only C, D, E). A `valid_input` containing 0 is incoherent with the sensitivity bands — the factor gets a valid letter while its product falls outside every band — and is not yet rejected.
- Non-regression guarantee: `sensitivity` keeps the config bands and is untouched — `_flatten_worker` over a real `tbl_stacked` partition before and after gives 0 differences across 2,723 cells on every sensitivity, min/max and count column. Existing `tbl_flat` files are not corrected retroactively; they carry the constant E until Process is run again.

## Unassessed values must not be clipped into the lowest bin (2026-07-30)

- Rule: `_compute_owa_counts_from_stacked` excludes sensitivity outside `min_score..max_score` from the histogram instead of clipping it in. A `0` is "not assessed" — the value asset import seeds for an unvalued group — not a real lowest-bin observation.
- Why: the clip turned every unvalued row into an `owa_n1` count, so each cell's `owa_n1` equalled its overlap count and `index_owa`'s lexicographic ranking ordered cells by how many assets touched them while presenting the result as a sensitivity index. On the Mombasa project every one of 37,184 stacked rows has sensitivity 0, yet `index_owa` spread 17..100 across 8,480 cells. The A–E code column was honest about the same input — null everywhere, because 0 falls outside every band — so one column admitted the data was missing while the other manufactured a plausible ranking from it. That asymmetry is the thing to notice: a fabricated number is worse than a null, because nothing downstream can tell it apart from a real one.
- How to apply: `clip()` on an input scale is almost always wrong where the scale has a meaning below its minimum. Ask what the out-of-range value means before folding it in. The count of dropped rows is logged, so an unassessed project now says so and yields `index_owa` 0 — which is what "nothing to rank" should look like.
- Non-regression guarantee: a no-op wherever every sensitivity is validly 1..25, which is every properly valued project. mesa-server took the same exclusion on 2026-07-30, in `worker/build_flat.sql`'s `owa_n*` counts, as the one deliberate exception to the 5.5.0 parity window that runs to 2026-08-04.

## A test that reads the repo's live project data is not a test (2026-07-30)

- Rule: `_flatten_worker` backfills from `tbl_asset_group` via `_get_asset_group_lookup()`, which resolves `gpq_dir()` from the working directory. Any test touching it must prime `pi._ASSET_GROUP_LOOKUP` with an empty frame, or it reads whatever project is sitting in `output/geoparquet`.
- Why: `test_flatten_worker_preserves_unique_asset_lists_and_scores` failed with `sensitivity_code_max == 'E'` on a row whose sensitivity is 25 — a value matching neither the fixture nor the ranges. The fixture's `ref_asset_group` values (10, 20, 30) collide with real ids in the repo's restored project, where id 10 is "Sand mining" with `sensitivity_code = E`, so the asset group's code was backfilled over the row's own. Considerable time went into hunting a selection bug in `_select_extreme_local` that does not exist; the helper is correct in isolation. It also explains why the suite went green in `73b3694` and red again with no code change in between: a restore on 2026-07-27 replaced `output/geoparquet` wholesale.
- How to apply: when a test fails with a value that appears in neither the fixture nor the expectation, suspect the environment before the logic — something is reading state the test did not set. Grep the unit under test for path resolution that starts from the working directory. `pytest tests/ -q` is the documented pre-release gate; it is not in CI, so it only protects a release if somebody runs it.
- Non-regression guarantee: isolating the lookup changed no production path. The suite is green at 4 passed, and its runtime fell from 5.66s to 1.18s once it stopped reading project data from disk.

## 3D landscape proportions follow the cell, not the scene (2026-08-05)

- Rule: `_landscape_max_height` in `qgis_results_project.py` caps the tallest column at `_LANDSCAPE_CELL_ASPECT` (4) times the grid cell width, computed per layer from the grid level. Height derived from the scene span alone is only correct for the coarsest grid in the project.
- Why: the height was computed once, outside the per-group loop, as 6% of the dataset span — 1733 m for a 28.9 km scene — and reused for every 3D view. Cell width across those views varies by a factor of 20, so QDGC_L11's 54 m cells got 1733 m columns: 32:1, and H3_R10 11:1. At that aspect a column is a pipe, its top face never turns toward the camera, and at scene distance the cell is under a pixel wide, so only the lit side walls register. The report was "hexagons look like walls without a roof". The geometry was never at fault — the cells are clean closed polygons and `rendered-facade="3"` (Walls|Roof) is correct in both QGIS 3 and 4. Cell width is derived from the level rather than measured (H3 edge = 1107.7 km / √7^r, QDGC = 2^-n degrees), so nothing reads geometry in the parent process.
- How to apply: whenever a visual property is scaled off the dataset extent, check it against the size of the feature it is drawn on. Extent-relative scaling silently assumes one feature size per project, and a multi-resolution grid breaks that assumption. Same class of check for symbol sizes, label offsets and buffer widths.
- Non-regression guarantee: the cap only ever lowers a column. The coarsest grid, whose cells are wide enough that the span still governs, is unchanged — H3_R8 stayed at 1733 m.

## Transparency in a 3D symbol is a render-pass decision, not a colour tweak (2026-08-05)

- Rule: treat `opacity` on a 3D phong material as a setting with a rendering cost, not a shade. It is now a named constant (`_LANDSCAPE_OPACITY`, 0.93) in `qgis_results_project.py` so it can be dialled back to 1 in one place if the columns start reading as hollow.
- Why: QGIS' forward render view has a dedicated transparent-objects pass whose render states include `QNoDepthMask` — geometry in that pass is drawn without depth writes, so faces blend in submission order and near faces stop occluding far ones. The first version of this generator carried `opacity="0.99"`, which is visually indistinguishable from opaque but semantically not opaque. It was suspected of causing the hollow-tube appearance, and that suspicion was written up here as fact; checking the QGIS source afterwards found the pass and its states but *no* code putting vector symbol entities into the transparent layer, so the mechanism is unconfirmed for polygon 3D symbols. The hollow look was fully explained by the aspect ratio (see the entry above) and 0.93 was added deliberately afterwards, at the user's request, with no return of the symptom.
- How to apply: when a near-opaque value shows up in generated style XML, ask whether the engine branches on `< 1` rather than on the visible difference — the answer decides which pass the geometry lands in. And when a plausible mechanism is inferred rather than read from the source, say which one it is before it hardens into a rule; this entry was corrected within the same session for exactly that.

## Outsized mosaic cells in 3D (2026-08-05)

- Rule: `basic_mosaic` cells past `_OUTSIZED_CELL_AREA_MULTIPLE` (100) times the group's median area render at `_OUTSIZED_OPACITY` in the generated 3D landscapes, keeping their height. The threshold is per group and computed from `area_m2`, so a regular grid never trips it.
- Why: the mosaic is cut from the asset footprints, so its cells inherit their size distribution. In the Kampala demo the median cell is 1,215 m² and the largest is 186 km² — 153,000× the median and 34% of the mosaic's whole area. Extruded to the A-class height it roofed over the entire scene: the 3D view showed one flat red plate with the fine structure hidden underneath. Measured across the same project, every H3 and QDGC group has max/median = 1.0, which is the point — a cap that is meaningless for a regular grid is exactly what an irregular one needs. Filtering the big cells out instead was considered and rejected: cells above the threshold are only 435 of 26,975 but carry 76% of the mosaic's area, so dropping them guts the map rather than tidying it.
- How to apply: when a per-feature visual encoding meets a layer whose features vary in size by orders of magnitude, check the max/median ratio before assuming one setting fits the layer. `area_m2` is already in `tbl_flat`, so this costs one more streamed column, no geometry.
- Caveat, not yet closed: whether QGIS honours sub-1 opacity for these entities is unverified — the transparent render pass exists but no code was found routing vector symbol entities into it (see the entry above). If the outsized cells still render solid in QGIS, the fallback is to exclude them from the 3D layers with the same threshold.

## A swallowed read is a delete (2026-08-06)

- Rule: never turn a failed read of a table into an empty frame when the caller writes that table back. `_load_existing_geocode_groups` in `geocode_manage.py` raises instead, and `_rewrite_geocode_objects` raises rather than writing a partial table.
- Why: `_load_existing_geocodes` caught every exception from `gpd.read_parquet(tbl_geocode_object)` and returned an empty GeoDataFrame. `_merge_and_write_geocodes` then concatenated the new grid objects onto nothing and wrote the result, so generating H3/QDGC levels silently deleted 17,853,399 basic_mosaic faces while leaving the group row in `tbl_geocode_group`. The project therefore looked like it had `basic_mosaic` registered and processing ran without it. Nothing was logged, because the swallow was the whole error path. The code dated from 2026-05-06 and had not changed: what was new was a mosaic large enough for the read to fail.
- How to apply: when reviewing a `try/except` around a read, ask what the caller does with the fallback value. A fallback that is written back is a delete with extra steps. The rule is narrower than "do not catch broadly" - catching is fine where the value is only displayed.
- Non-regression guarantee: the group table's own read is unchanged for the case that matters, a project with no geocodes yet, which is handled by the `exists()` check rather than the exception path.

## Merging geocode objects (2026-08-06)

- Rule: `tbl_geocode_object` is merged by streaming row groups through pyarrow, never by loading it into geopandas. `_rewrite_geocode_objects` drops whole groups with an Arrow filter and appends the new objects, leaving geometry as the WKB it already is on disk.
- Why: decoding the table into shapely costs 1,269 bytes a row, measured on the Albertine Graben project - 23 GB for 19.4 million rows, before `pd.concat` copies it again. The machine has 127 GB but had 64 GB free while a mosaic build was running. Streaming the same merge, adding 250,000 objects to the real 2.3 GB table, took 15 s at a peak of 0.50 GB.
...
- Why: an operator pressed "Create report" twice, eight seconds apart (11:48:08 and 11:48:16). Nothing stopped the second click, so two daemon threads rendered the same report in parallel — visible in log.txt as every line doubled, with the two runs diverging on the numbers ("130 cached, 24 fetched" against "133 cached, 21 fetched" for the same basemap). They shared the scratch files in `output/tmp/` and then both composed into `MESA-report_2026-09-16_1155.docx`, because the name is stamped to the minute. The interleaved writes produced a zip whose local headers came from one run and whose compressed streams came from the other: 26 of 41 entries failed CRC, 22 would not inflate at all, and Word refused the file. Eight minutes of rendering, unrecoverable — a salvage attempt got `word/document.xml` out intact and nothing else.
- How to apply: when adding or reviewing a button that starts a background render, check three things — is a second press refused, does the output path collide if two runs land in the same minute, and do the scratch files in `output/tmp/` carry a per-run identity. A corrupt output is the failure mode here, not a lost race: the loser does not simply lose, both writers destroy the file.
- Non-regression guarantee: a single run behaves exactly as before, the filename keeps its `MESA-report_YYYY-MM-DD_HHMM.docx` form, and the `_2`, `_3` suffix only appears when that exact name is already on disk.

## Generated geocode names are zero-padded (2026-09-16)

- Rule: name generated grids through `h3_group_name()` / `qdgc_group_name()` in `mesa_shared.py` (`H3_R07`, `QDGC_L06`), never with an inline f-string. Anything that picks "the first" group by sorting relies on the padding.
- Why: the unpadded names sorted as text, so `H3_R10` came before `H3_R7` in the report's map selection and the maps. The same order silently inverted `choose_primary_geocode`: its docstring promised the coarsest H3 grid as the fallback when `basic_mosaic` is absent, and with R6 and R10 present it returned R10 — the finest and most expensive grid. The docstring described intent; nothing tested it. The group name is a join key (`name_gis_geocodegroup`), so the rename needed a migration, not just a new f-string: without one, regenerating a level writes `H3_R07` beside the old `H3_R7`, because generation replaces groups by name.
- How to apply: `migrate_geocode_group_names()` runs at startup, after every restore and before H3/QDGC generation. It is gated on a cheap check (the group table's names, two directory listings, config.ini) and rewrites `tbl_geocode_group` last, so that table is the marker — an interrupted run leaves the gate open and finishes next time. When adding a table, file or config key that stores a grid name, check the migration reaches it: whole-value string columns anywhere under geoparquet are covered, group-named files only in `tbl_segmentation/` and `output/mbtiles/`, free text only in `tbl_settings` and config.ini values. A name embedded inside a longer value is deliberately not touched.
- Non-regression guarantee: run on a copy of a real 5.6 workspace (11 groups, 69 MBTiles), 24 tables compared equal before and after once legacy names are mapped — row counts, every other column, and the schema-level `geo` header — in 0.50 s. H3 cell indexes and QDGC cell codes are unchanged; only the label is MESA's.

## px font sizes make QFont.pointSize() -1 (2026-09-23)

- Rule: sizing every widget in px is a deliberate choice and stays — but it means `QFont.pointSize()` returns `-1` app-wide, so Qt's own relative font scaling emits `QFont::setPointSize: Point size <= 0 (-1)`. `ui_style.install_font_warning_filter()` drops that one message; do not chase it as a bug, and do not silence anything else with it.
- Why: `ASSET_STYLESHEET` and `MESA_STYLESHEET` both set `font-size: 11px`, and a QSS px size sets the font's *pixel* size, leaving point size undefined (-1). Wherever Qt scales a font relatively — combo-box and menu popups do, on Windows and macOS alike — it multiplies that -1 and hands the result back to `setPointSize`/`setPointSizeF`. Qt rejects it and changes nothing: verified that a `QFont` with `pixelSize=11` still reports `pixelSize=11` after `setPointSize(-1)`, so the px size stands and nothing renders wrong. Reproduced offscreen with the shared stylesheet alone (three warnings, from a `QComboBox` popup and a `QMenu`); MESA's own code never calls `setPointSize` at all.
- How to apply: when this message reappears, check it is still only this cause before widening the filter — the handler chains to the previous one, so every other Qt warning (including `QFontDatabase` complaints) still reaches stderr. Converting the stylesheets from px to pt would remove the cause, but changes text size in every window and interacts with per-platform DPI scaling; that is a deliberate visual change, not a warning fix.
- Non-regression guarantee: no font, size or stylesheet value changed — the filter only suppresses a log line, and is installed from `apply_shared_stylesheet()` so helper windows and the main window get identical behaviour on both platforms.

## tbl_asset_group must stay GeoParquet (2026-09-23)

- Rule: never write `tbl_asset_group.parquet` from a plain pandas DataFrame. `asset_manage.save_asset_group_df()` now forces every write through `_as_geodataframe()`, which rebuilds shapely geometries from WKB and re-attaches the working CRS. A pandas `to_parquet` writes no `geo` metadata, and geopandas then refuses the file outright.
- Why: `load_asset_group_df()` read the table with `pd.read_parquet`, which turns the bbox polygons into opaque WKB bytes, and `save_asset_group_df()` wrote that straight back. Every Save in the Edit-assets tab — and, from 5.7, every Delete asset — therefore stripped the file's geo metadata while leaving the rows perfectly intact. The damage is invisible until something reads it with geopandas: atlas generation failed with `Failed reading tbl_asset_group.parquet: Missing geo metadata in Parquet/Feather file`, then `Cannot derive atlas extent`, because `_read_asset_group_parquet()` swallows the error and returns an empty frame. `processing_setup.py` had quietly grown `_load_asset_group_without_geo_metadata()` for the same corruption — a downstream workaround that hid the real writer for however long it had been there.
- How to apply: when a table holds geometry, the reader and the writer must agree on it. If a code path reads with `pd.read_parquet` for convenience, its writer owes the geometry column a reconstruction before writing. A repair needs no re-import: the WKB is still in the file, so decoding it and re-writing with a CRS restores the table exactly — verified on a 9-row table, every non-geometry column equal and every geometry equal.
- Non-regression guarantee: the import path already wrote real GeoDataFrames and is untouched; `load_asset_group_df()` still opens files that lack geo metadata, via a pandas fallback, so an older workspace loads and is healed by its next save.

## Progress map must show what is actually computed (2026-09-24)

- Rule: a progress display fed from `imap_unordered` keys on the index each result returns, never on the count of results so far; and "done" on the map means the area's result exists in `tbl_flat`, not that an intermediate stage finished.
- Why: the processing minimap painted chunks `1..done_count` green and the next `max_workers` as running. Chunks finish out of order (dense asset areas take far longer), so a heavy chunk could show green while still intersecting and a finished one show queued — right count, wrong places. It also set every cell to done when intersect ended; flatten, where sensitivity is actually computed, never touched the cells, so the whole flatten stage showed a fully green map.
- How to apply: intersect marks the returned chunk's cells `intersected` (or `done` if it wrote no parts) and records `part_chunks` (partition basename → chunk) and `chunk_cells` in `__chunk_status.json`; `_FlattenMinimapTracker` turns a chunk's cells `done` when all its partitions are flattened. Any writer of the status file must carry existing keys (`{**existing, ...}`) or a later flatten-only run loses the maps. When adding a stage that works per area, give it its own map state rather than reusing `done`.
- Non-regression guarantee: without the maps (status file from an older run) the tracker is inert and the end of flatten marks every cell done, as before.

## GeoNode/GeoServer REST: two silent failures in map export (2026-09-24)

- Rule: in a GeoNode 5 map PATCH, `maplayers[].dataset` is the dataset **pk** (an int), never a dict; and against GeoServer 2.27, a PUT to a style that does not exist answers **400**, not 404, so check existence (`GET …/styles/<name>.json`) before deciding to POST.
- Why: every map MESA created was empty. The PATCH carrying the MapStore config and maplayers sent `{"dataset": {"alternate": …}}`; dynamic_rest resolves the relation with `objects.get(pk=data)` and raised `TypeError: Field 'id' expected a number` → HTTP 500, and because the view is `@transaction.atomic` the `data` blob was dropped too. The old single "MESA Sensitivity Map" had the same bug. Separately, the four alternative result styles were never created: the create branch only ran on 404, GeoServer returned 400, so each layer referenced `geonode:<name>_index_owa` etc. and WMS answered `StyleNotDefined`. The export still logged "Done", so the operator found out by opening a blank map.
- How to apply: when calling GeoNode's API, read the serializer on the server (`geonode/maps/api/serializers.py`) rather than guessing field shapes; relation fields take pks. After any GeoServer style push, verify with a WMS GetMap using that style — a 200 with `application/vnd.ogc.se_xml` is a failure. A map whose layer PATCH fails is deleted rather than left empty in the catalogue.
